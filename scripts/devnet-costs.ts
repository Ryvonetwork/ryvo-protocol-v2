...
(async () => {
  ...
  for (let i = 0; i < sigs.length && !stop; i += 20) {
    ...
    for (const tx of txs) {
      ...
      const label = mine.includes("SealAndQueueRoute")
        ? "seal_and_queue (+tail stage_slots)"
        : mine.includes("SettleChannels")
        ? "settle_channels"
        : mine.includes("StageRecords") && mine.includes("ResetStaging")
        ? "stage_records (+reset_staging)"
        : mine.includes("StageRecords")
        ? "stage_records"
        : mine.includes("StageChannels")
        ? "stage_channels"
        : mine.includes("ResetStaging")
        ? "stage_slots (+reset_staging)"
        : mine.includes("StageSlots")
        ? "stage_slots"
        : mine.includes("CloseStaging")
        ? "close_staging"
        : "open_staging";
  ...
  let tFee = 0,
    ...
    tN = 0;
  console.log(
    "| instruction | tx | fees (SOL) | Arcium fee (SOL) | total paid (SOL) | CU total | CU/tx |"
  );
  console.log("|---|---|---|---|---|---|---|");
  for (const r of [...rows.values()].sort((a, b) => b.n - a.n)) {
    console.log(
      `| ${r.kind} | ${r.n} | ${(r.fee / 1e9).toFixed(6)} | ${(
        r.arciumFee / 1e9
      ).toFixed(6)} | ${(r.delta / 1e9).toFixed(
        6
      )} | ${r.cu.toLocaleString()} | ${Math.round(
        r.cu / r.n
      ).toLocaleString()} |`
    );
    tFee += r.fee;
  ...
  console.log(
    `| TOTAL | ${tN} | ${(tFee / 1e9).toFixed(6)} | | ${(tDelta / 1e9).toFixed(
      6
    )} | ${tCu.toLocaleString()} | |`
  );
})();
