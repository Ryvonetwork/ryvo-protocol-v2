...
(async () => {
  ...
  console.log("rpc:", url.replace(/api-key=.*/, "api-key=…"), "| node:", v["solana-core"], "| feature-set:", v["feature-set"]);
  const feat = await c.getAccountInfo(FEATURE);
  console.log("enable_tx_v1 feature account:", feat ? `EXISTS (${feat.data.length} bytes)` : "absent (not activated)");
  ...
  const wire = buildV1Transaction(
    ...
    [kp], blockhash, { computeUnitLimit: 20_000 },
  );
  const mySig = bs58(wire.subarray(wire.length - 64));
  let rpcSig = "";
  try {
    rpcSig = await c.sendRawTransaction(wire, { skipPreflight: true, maxRetries: 3 });
  } catch (e: any) {
    console.log("sendRawTransaction rejected:", String(e.message).slice(0, 200));
    return;
  }
  console.log("rpc-returned sig:", rpcSig, "\nreal sig (last 64B):", mySig, rpcSig === mySig ? "(same)" : "(DIFFERENT — RPC did not decode v1)");
  for (let i = 0; i < 40; i++) {
    await new Promise((r) => setTimeout(r, 1500));
    const st = await c.getSignatureStatuses([mySig, rpcSig], { searchTransactionHistory: true });
    const [a, b] = st.value;
    if (a || b) { console.log("LANDED:", JSON.stringify(a ?? b)); return; }
    const h = await c.getBlockHeight("confirmed");
    if (h > lastValidBlockHeight) { console.log("expired without landing (block height exceeded) — validators did not accept it"); return; }
  }
