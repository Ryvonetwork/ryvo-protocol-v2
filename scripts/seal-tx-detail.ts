...
(async () => {
  ...
  for (const s of sigs) {
    ...
    if (!logs.some((l) => l.includes("Instruction: SealAndQueueRoute"))) continue;
    console.log("tx", s.signature, "fee", tx.meta.fee, "CU", tx.meta.computeUnitsConsumed);
    const keys = tx.transaction.message.staticAccountKeys;
    keys.forEach((k, i) => {
      const d = tx.meta!.postBalances[i] - tx.meta!.preBalances[i];
      if (d !== 0) console.log(`  ${k.toBase58()}  ${d > 0 ? "+" : ""}${d} lamports`);
    });
    const inner = tx.meta.innerInstructions ?? [];
    for (const ii of inner) for (const ix of ii.instructions) {
      const pid = keys[ix.programIdIndex];
      if (pid.equals(new PublicKey("11111111111111111111111111111111"))) console.log("  system ix accounts:", ix.accounts.map((a) => keys[a].toBase58()).join(", "));
    }
    ...
    for (let i = 0; i < keys.length; i++) {
      ...
      if (d > 0) {
        ...
        console.log(`  ${keys[i].toBase58()} now: ${info ? `${info.lamports} lamports, ${info.data.length} bytes, owner ${info.owner.toBase58()}` : "closed"}`);
      }
    }
    break;
  }
