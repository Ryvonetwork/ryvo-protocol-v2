...
(async () => {
  ...
  for (let i = 0; i < sigs.length; i += 25) {
    ...
    for (const tx of txs) {
      ...
      const progs = new Set(tx.transaction.message.compiledInstructions.map((ix) => keys[ix.programIdIndex].toBase58()));
      if (progs.size !== 1 || !progs.has("11111111111111111111111111111111")) continue;
      const w = keys.findIndex((k) => k.equals(wallet));
      if (tx.meta.postBalances[w] >= tx.meta.preBalances[w]) continue; // not outgoing
      keys.forEach((k, i) => { if (i !== w && tx.meta!.postBalances[i] > tx.meta!.preBalances[i]) recipients.add(k.toBase58()); });
    }
  }
  ...
  for (let i = 0; i < list.length; i += 100) {
    ...
    for (const info of infos) if (info && info.lamports > 0) { total += info.lamports; nonEmpty++; }
  }
  console.log(`${list.length} funded party keypairs; ${nonEmpty} still hold SOL: ${(total / 1e9).toFixed(4)} SOL (keys were random and discarded — unrecoverable)`);
})();
