...
(async () => {
    ...
    for (const tx of txs) {
      ...
      if (
        !(tx.meta.logMessages ?? []).some((l) =>
          /Instruction: SealAndQueue/.test(l)
        )
      )
        continue;
      ...
      for (const ix of tx.transaction.message.compiledInstructions) {
        if (!PROGRAMS.includes(keys[ix.programIdIndex]) || ix.data.length < 16)
          continue;
        const offset = new anchor.BN(
          Buffer.from(ix.data).subarray(8, 16),
          "le"
        );
        ...
        try {
          ...
          claimed += info.lamports;
          console.log(
            `claimed computation ${comp.toBase58()} ${info.lamports}`
          );
        } catch (e: any) {
          console.log(
            `kept computation ${comp.toBase58()}: ${String(e?.message).slice(
              0,
              100
            )}`
          );
  ...
  try {
    ...
    const bufs = await connection.getProgramAccounts(program.programId, {
      dataSlice: { offset: 0, length: 0 },
      filters: [
        { memcmp: { offset: 0, bytes: disc.bytes as string } },
        { memcmp: { offset: 16, bytes: payer.publicKey.toBase58() } },
      ],
    });
    for (const b of bufs) {
      try {
        await closeStaging(program, payer, b.pubkey);
        console.log("closed staging", b.pubkey.toBase58());
      } catch (e: any) {
        console.log(
          "kept staging",
          b.pubkey.toBase58(),
    ...
    console.log("staging scan skipped:", String(e?.message).slice(0, 80));
  }
  ...
  for (const name of ["clear_unilateral", "clear_route"]) {
    ...
    if (!info) {
      console.log(`comp def ${name}: gone`);
      continue;
    }
    const cd: any = await (
      arcium.account as any
    ).computationDefinitionAccount.fetch(pda);
    if (cd.deactivationSlot === null || cd.deactivationSlot === undefined) {
      try {
        await (arcium.methods as any)
          .deactivateComputationDefinition(off, live)
          .accountsPartial({ signer: payer.publicKey, mxe, compDefAcc: pda })
          .rpc({ commitment: "confirmed" });
        console.log(
          `comp def ${name}: deactivated (TTL ~180 slots before close)`
        );
      } catch (e: any) {
        console.log(
          `comp def ${name}: deactivate failed: ${String(e?.message).slice(
            0,
            120
          )}`
        );
    ...
    try {
      await (arcium.methods as any)
        .closeComputationDefinition(off, live, clusterOffset)
        .accountsPartial({
      ...
      console.log(
        `comp def ${name}: close failed: ${String(e?.message).slice(0, 140)}`
      );
    }
  }

  // 4. lookup tables: deactivate, wait out the cooldown (~513 slots), close
  const alts = await connection.getProgramAccounts(ALT, {
    filters: [{ memcmp: { offset: 22, bytes: payer.publicKey.toBase58() } }],
  });
  console.log(
    `${alts.length} lookup tables to close (${(
  ...
  for (const a of alts) {
    ...
    if (st && st.deactivationSlot === BigInt("0xffffffffffffffff"))
      toDeactivate.push(a.pubkey);
  }
  for (let i = 0; i < toDeactivate.length; i += 8) {
    const tx = new Transaction();
    for (const k of toDeactivate.slice(i, i + 8))
      tx.add(
  ...
  if (toDeactivate.length)
    console.log(
      `deactivated ${toDeactivate.length} lookup tables; waiting for the cooldown`
    );
  ...
  for (let attempt = 0; attempt < 40 && remaining.length; attempt++) {
    ...
    for (let i = 0; i < remaining.length; i += 8) {
      ...
      try {
        await provider.sendAndConfirm(tx, []);
        console.log(`closed ${chunk.length} lookup tables`);
      } catch {
        still.push(...chunk);
      }
    }
  ...
  if (remaining.length)
    console.log(
      `${remaining.length} lookup tables still in cooldown — rerun later`
    );

  const end = await connection.getBalance(payer.publicKey);
  console.log(
    `net recovered this run: ${((end - start) / 1e9).toFixed(
      4
    )} SOL; balance ${(end / 1e9).toFixed(4)} SOL`
  );
})();
